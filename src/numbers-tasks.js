...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const xyLength = Math.sqrt(x1 * x1 + y1 * y1);
  const yxLength = Math.sqrt(x2 * x2 + y2 * y2);

  return Math.acos(scalar / (xyLength * yxLength));
}
...
function getSumToN(n) {
  const numbers = new Array(n).fill(n);
  const numToSum = numbers.map((num, index) => num - index);
  return numToSum.reduce((acc, cur) => acc + cur);
}
